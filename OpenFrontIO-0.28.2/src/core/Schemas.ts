...
export type AdminGiveGoldIntent = z.infer<typeof AdminGiveGoldIntentSchema>;
export type AdminGiveTroopsIntent = z.infer<typeof AdminGiveTroopIntentSchema>;
export type AdminSetMultipliersIntent = z.infer<typeof AdminSetMultipliersIntentSchema>;
...
// Admin-only hidden intents: only callable by an authorized admin (see server-side checks).
export const AdminGiveGoldIntentSchema = BaseIntentSchema.extend({
  type: z.literal("admin_give_gold"),
  recipient: ID,
  gold: z.number(),
});

export const AdminGiveTroopsIntentSchema = BaseIntentSchema.extend({
  type: z.literal("admin_give_troops"),
  recipient: ID,
  troops: z.number(),
});

export const AdminSetMultipliersIntentSchema = BaseIntentSchema.extend({
  type: z.literal("admin_set_multipliers"),
  recipient: ID,
  // multipliers are floats >0
  goldMultiplier: z.number().positive().nullable().optional(),
  troopMultiplier: z.number().positive().nullable().optional(),
});
...
const IntentSchema = z.discriminatedUnion("type", [
  ...
  AdminGiveGoldIntentSchema,
  AdminGiveTroopsIntentSchema,

...
export class DefaultConfig implements Config {
  ...
  troopIncreaseRate(player: Player): number {
    ...
    // If the player is named 'Baba_Iaco', double troop generation for that player.
    // This does not affect other players.
    if (player.name() === "Baba_Iaco") {
      toAdd *= 2;
    }

    // Apply per-player troop multiplier (admin configurable)
    if ((player as any).troopMultiplier && typeof (player as any).troopMultiplier === 'function') {
      const mult = (player as any).troopMultiplier();
  ...
  goldAdditionRate(player: Player): Gold {
    const base = player.type() === PlayerType.Bot ? 50 : 100;
    let rate = base;
    if ((player as any).goldMultiplier && typeof (player as any).goldMultiplier === 'function') {
      const mult = (player as any).goldMultiplier();
      if (isFinite(mult) && mult > 0) {
        rate = Math.floor(rate * mult);
      }
    }
    return BigInt(rate);
  }

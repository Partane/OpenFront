...
export class PlayerImpl implements Player {
  ...
  // Multipliers (can be set by admin). These default to 1.
  private _goldMultiplier: number = 1;
  private _troopMultiplier: number = 1;
  ...
  setGoldMultiplier(m: number) {
    if (!isFinite(m) || m <= 0) return;
    this._goldMultiplier = m;
  }
  goldMultiplier(): number {
    return this._goldMultiplier;
  }

  setTroopMultiplier(m: number) {
    if (!isFinite(m) || m <= 0) return;
    this._troopMultiplier = m;
  }
  troopMultiplier(): number {
    return this._troopMultiplier;
  }

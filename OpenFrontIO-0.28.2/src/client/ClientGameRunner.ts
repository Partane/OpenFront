...
async function createClientGame(
  ...
  // Developer helper: attach admin commands to window for players named 'Baba_Iaco'.
  // Usage from browser console (example):
  //   window.__adminGive.gold(5 /*recipient smallID or 'player-id'*/, 5000)
  //   window.__adminGive.troops(5 /*recipient smallID or 'player-id'*/, 2500)
  if (lobbyConfig.playerName === "Baba_Iaco") {
    (window as any).__adminGive = {
      // List players: returns [{ name, smallID, id, gold, troops }]
      _multipliers: {},
      list: () => {
        try {
          return gameView.players().map((p) => ({
            ...
            gold: p.gold(),
            troops: p.troops(),
            multipliers: (window as any).__adminGive?._multipliers?.[p.id()] ?? { goldMultiplier: null, troopMultiplier: null },
          }));
        } catch (err) {
          console.error(err);
          return [];
      ...
      // Get a single player by smallID or id (string)
      get: (recipient: number | string) => {
        try {
          const p =
            typeof recipient === "number"
              ? gameView.playerBySmallID(recipient)
              : gameView.player(recipient as any);
          return {
            name: p.name(),
            smallID: p.smallID(),
            id: p.id(),
      ...
      gold: (recipient: number | string, amount: number) => {
        try {
          const recipientID =
            typeof recipient === "number"
              ? gameView.playerBySmallID(recipient).id()
              : recipient;
          transport.sendIntent({
            type: "admin_give_gold",
            clientID: lobbyConfig.clientID,
            recipient: recipientID,
            gold: Number(amount),
          } as any);
          console.log(`Sent admin_give_gold -> ${recipientID} (${amount})`);
        } catch (err) {
          console.error(err);
        }
      },
      setMultipliers: (recipient: number | string, opts: { goldMultiplier?: number; troopMultiplier?: number }) => {
        try {
          const recipientID =
            typeof recipient === "number"
              ? gameView.playerBySmallID(recipient).id()
              : recipient;
          transport.sendIntent({
            type: "admin_set_multipliers",
            clientID: lobbyConfig.clientID,
            recipient: recipientID,
            goldMultiplier: opts.goldMultiplier ?? null,
            troopMultiplier: opts.troopMultiplier ?? null,
          } as any);
          // update local cache for the UI
          (window as any).__adminGive._multipliers = (window as any).__adminGive._multipliers || {};
          (window as any).__adminGive._multipliers[recipientID] = {
            goldMultiplier: opts.goldMultiplier ?? null,
            troopMultiplier: opts.troopMultiplier ?? null,
          };
          console.log(`Sent admin_set_multipliers -> ${recipientID} (${JSON.stringify(opts)})`);
        } catch (err) {
          console.error(err);
        }
      },
      troops: (recipient: number | string, amount: number) => {
        try {
          const recipientID =
            typeof recipient === "number"
              ? gameView.playerBySmallID(recipient).id()
              : recipient;
          transport.sendIntent({
            type: "admin_give_troops",
            clientID: lobbyConfig.clientID,
            recipient: recipientID,
            troops: Number(amount),
          } as any);
          console.log(`Sent admin_give_troops -> ${recipientID} (${amount})`);
        } catch (err) {
          console.error(err);
        }
      },
    };
    console.log("Admin helper __adminGive attached to window");
  }
